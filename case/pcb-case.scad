...
module PCBLid(x, y, top_thickness, wall_thickness) {
  pcb_padding = 2;
  pcb_x = x + pcb_padding;
  pcb_y = x + pcb_padding;
  radius = 5;
...
module PCBCase(x, y, bottom_thickness, wall_thickness, peg_height = 6, pin_diameter = 2.3) {
  pcb_padding = 2;
  pcb_x = x + pcb_padding;
  pcb_y = y + pcb_padding;
  ...
  radius = 5;
  pcb_z = 1.6; // pcb
  pcb_thickness = pcb_z + 8.4; // pcb + components
  ...
  translate([0, 0, bottom_thickness]) {
    difference() {
      ...
      translate([0, 0, -bottom_thickness])
        ...
        translate([0, -((pcb_y + wall_thickness) / 2), peg_height + pcb_z - (usb_padding / 2)])
          linear_extrude(usb_height + usb_padding) // 3.2mm high + 2mm padding
          // 2mm padding so it looks nice in preview
          square(size = [usb_width, wall_thickness + 2], center = true);
    }
